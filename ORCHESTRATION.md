...
유료 플랜이 바닥나면 **로컬 모델이 이어받는다**(런처 토글 `sag local`). 로컬은 일상 작업 ~80%를 처리 — 최난도는 유료 복구까지 대기. → `FALLBACK.md`
...
- **확인(예) →** `sag-run "<작업>"`으로 로컬 워커에 위임(세션은 그대로 클로드 유지) → 결과를 **검증**(약한 모델이므로 맹신 금지) 후 적용.
- **금지**: 확인 없는 자동 위임. 어려운 판단·아키텍처·미묘한 로직은 위임하지 않는다.
- 생성형 작업은 `sag-run`으로 바로. 파일 편집형 대량작업은 별도 로컬 세션(`sag local`) 고려.
