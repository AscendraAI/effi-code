...
그래서 구독 사용자의 정답은 **런처 토글** — 프록시 없이, 한도 도달 시 로컬로 스위치. (아래 `sag`)

> ✅ 핵심 메커니즘 실측 검증됨: Ollama가 Anthropic Messages API 네이티브 지원(v0.14+)이라
> 로컬 모델이 클로드 백엔드로 그대로 응답. `sag local` 이 이걸 env로 꽂아준다.
...
```bash
# PATH에 넣기 (1회)
ln -s "$PWD/bin/sag" /opt/homebrew/bin/sag    # 또는 export PATH="$PWD/bin:$PATH"

sag            # 평소 = CLOUD (구독 Claude)
sag local      # 한도 소진 시 → 로컬(무료)로 Claude Code 실행
sag status     # 현재 로컬 모델/Ollama 상태
```

한도 메시지를 보면 `sag local`로 이어서 작업 → 유료 복구되면 새 창에서 `sag`(cloud).
`sag local`은 Ollama 자동 기동 + **모델 메모리 유지(keep-alive)**로 로드 지연도 없앤다.
...
24GB면 `SA_LOCAL_MODEL=devstral sag local`. 더 빠르게: `brew upgrade ollama`(0.30+는 MLX 백엔드로 Apple Silicon 가속).
...
## 반드시 실측 (fail-closed)
`sag local`로 실제 응답이 오는지 1회 확인 (핵심 엔드포인트는 이미 검증됨 — 위 curl).
...
파묻혀 "안녕"에도 랜덤 툴콜을 뱉는다(실측 2026-07). 그래서 `sag local`은 `--strict-mcp-config`로
